...
// ── the same campaign link, handed in twice ──────────────────────────
//
// Not a theoretical case. A cold start from a campaign link delivers the
// URL twice — `continue userActivity` and the app's own router both see it
// — and most routers call the handler again on every redirect evaluation.
// The de-duplication used to compare against the *pending* click id, which
// is deleted as soon as it has been attached to an event, so the guard
// stopped working at the moment it had done its job: the second call
// reported a second session and re-credited the campaign that produced the
// install.
//
// Driven through the real `Notibase` surface rather than a pure helper,
// because the bug was in which stored value it read, not in the
// comparison. The count is asserted on the database side, in
// e2e-ios-core.mjs.
print("[swift] a repeated deep link is acted on once")
let defaults = UserDefaults.standard
// The state a deep link actually arrives in: a device already registered,
// the install already reported. Cleared first so a re-run is a fresh run.
defaults.set(deviceId, forKey: "nb_device_id")
defaults.set(true, forKey: "nb_install_reported")
defaults.removeObject(forKey: "nb_last_click")
defaults.removeObject(forKey: "nb_pending_click")
// If this fails, nothing below means anything: the SDK keeps every piece of
// its state in UserDefaults, so a run where it does not persist would fail
// the database assertions for a reason that has nothing to do with them.
check(defaults.string(forKey: "nb_device_id") == deviceId, "UserDefaults is usable here")

Notibase.configure(clientKey: clientKey, apiUrl: apiUrl)
// configure reports the cold start; that one carries no click id and is not
// what is being counted.
Thread.sleep(forTimeInterval: 0.6)

Notibase.handleDeepLink(url("darlivo://open?nb_click=1742"))
Notibase.handleDeepLink(url("darlivo://open?nb_click=1742"))
// The same id arriving on a different URL is still the same click.
Notibase.handleDeepLink(url("https://darlivo.test/l?nb_click=1742"))
// A genuinely different campaign still gets through: this de-duplicates, it
// does not latch shut.
Notibase.handleDeepLink(url("darlivo://open?nb_click=1743"))
Thread.sleep(forTimeInterval: 2.0)
check(defaults.string(forKey: "nb_last_click") == "1743", "the last click acted on is remembered")
