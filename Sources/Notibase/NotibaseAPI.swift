...
public final class NotibaseAPI {
    ...
    /// POST /v1/unsubscribe — stop sending to this device.
    ///
    /// A durable opt-out recorded against the token, not a flag on a row:
    /// it survives re-registration and reinstalls and is lifted only from
    /// your own backend with a server key.
    public func unsubscribe(deviceId: String, reason: String) throws {
        _ = try request(method: "POST", path: "/v1/unsubscribe",
                        body: ["device_id": deviceId, "reason": reason])
    }

    /// POST /v1/logout — this device is no longer that person's.
    public func logout(deviceId: String) throws {
        _ = try request(method: "POST", path: "/v1/logout",
