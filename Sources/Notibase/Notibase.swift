...
public enum Notibase {
    ...
    /// to anybody, so a campaign aimed at a person no longer reaches it,
    /// and the inbox stops returning the last person's messages, which
    /// matters on a shared phone. The next `identify` attaches it again.
    ///
    /// **Not `unsubscribe`.** That records a durable opt-out against the
    /// APNs token which survives sign-in, reinstall and re-registration,
    /// and is lifted only from your own backend. Reaching for it on
    /// sign-out silences the phone for whoever signs in next, permanently,
    /// with nothing in the app able to undo it.
    ///
    ...
    public static func logout(completion: ((Bool) -> Void)? = nil) {
        // In-app messages, and so the triggers, exist only where there is a
        // UI to draw them in — the same condition the whole in-app section
        // of this file is compiled under.
        #if canImport(UIKit) && !os(watchOS)
        clearTriggers()
        #endif
        guard let api = api else { logNotConfigured(); completion?(false); return }
        guard let deviceId = defaults.string(forKey: deviceIdKey) else {
            completion?(true)          // never registered, nothing to detach
            return
        }
        queue.async {
            do {
                try api.logout(deviceId: deviceId)
                completion?(true)
            } catch {
                NSLog("[Notibase] logout failed: %@", String(describing: error))
                completion?(false)
            }
        }
    }
    ...
    ///
    /// A durable opt-out, recorded against the APNs token: it survives
    ...
    public static func unsubscribe(reason: String = "user_request",
                                   completion: ((Bool) -> Void)? = nil) {
        guard let api = api else { logNotConfigured(); completion?(false); return }
        guard let deviceId = defaults.string(forKey: deviceIdKey) else {
        ...
        queue.async {
            do {
                try api.unsubscribe(deviceId: deviceId, reason: reason)
                completion?(true)
            } catch {
                NSLog("[Notibase] unsubscribe failed: %@", String(describing: error))
                completion?(false)
            }
        }
    }
    ...
    /// Set by `pauseInAppMessages()`. Main thread only, like everything here.
    private static var inAppPaused = false
    ...
    ///
    /// A message is drawn as soon as there is a window to draw it in, and on
    /// most apps the first window is a splash or a login screen. Nothing in
    /// this SDK can tell a splash from a real screen; only your app knows
    /// when it has finished starting up.
    ///
    /// ```swift
    /// Notibase.pauseInAppMessages()          // in didFinishLaunching
    /// Notibase.enableInAppMessages()
    ///
    /// // …once your first real screen is on screen:
    /// Notibase.resumeInAppMessages()
    /// ```
    ///
    /// Nothing is lost while paused. Rules are still fetched and a message
    /// stays eligible; it is held, and `resumeInAppMessages()` evaluates
    /// immediately. Nothing is counted either — an impression is recorded
    /// when a message is drawn, so a held message has not spent its
    /// frequency cap and reports no display nobody saw.
    ///
    /// Per process, not remembered across launches.
    @available(iOS 13.0, *)
    public static func pauseInAppMessages() {
        onMain { inAppPaused = true }
    }

    /// Let in-app messages show again, and evaluate now.
    ///
    /// Safe when nothing was paused, and safe more than once — a message
    /// already on screen is not disturbed.
    @available(iOS 13.0, *)
    public static func resumeInAppMessages() {
        onMain {
            if !inAppPaused { return }
            inAppPaused = false
    ...
    ///
    /// Main thread only, like everything else that touches this state.
    @available(iOS 13.0, *)
    public static var inAppMessagesPaused: Bool { inAppPaused }
    ...
    private static func evaluateInAppMessages() {
        if inAppShowing || inAppPaused { return }
