...
…and pick version **0.9.0** (or "Up to Next Major"). Or in `Package.swift`:

```swift
.package(url: "https://github.com/notibaseorg/notibase-ios.git", from: "0.9.0")
// product: "Notibase"
```
...
Notibase.logout()          // sign-out: the device stops being theirs
Notibase.unsubscribe()     // "turn off notifications": a durable opt-out
```
...
A message is drawn as soon as there is a window to draw into, and at startup
that window is your splash screen. Nothing here can tell a splash from a real
screen, so your app says when it is ready:

```swift
Notibase.pauseInAppMessages()             // in didFinishLaunching
Notibase.resumeInAppMessages()            // once your first real screen is up
```
