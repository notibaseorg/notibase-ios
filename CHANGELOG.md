...
- No changes. The mobile SDKs share one version number and 0.9.3 exists to
  give Android a release that builds — see its changelog. This SDK's 0.9.2
  tag is fine and 0.9.3 is identical to it.

## 0.9.2

- No changes. The mobile SDKs share one version number, and 0.9.2 is a Flutter
  fix: `attachFirebase()` there awaited a device registration in front of
  `runApp()`. This SDK has never done that — `configure()` queues the work and
  returns — so there is nothing here to change.

## 0.7.5

No change to this package beyond the version. Published alongside the Flutter
SDK, whose in-app messages were not appearing on a cold start — and the three
mobile SDKs share one version number.

## 0.7.4

No change to this package beyond the version. Published alongside the Flutter
SDK, which was reading a simulator's longer APNs token as a Firebase one —
and the three mobile SDKs share one version number.

## 0.7.3

No change to this package beyond the version. Published alongside the Flutter
SDK, whose registration log now names the push service a token belongs to —
and the three mobile SDKs share one version number.

## 0.7.2

No change to this package beyond the version. Published alongside the Flutter
SDK, which stopped one phone becoming two audience members when its push
token changes — and the three mobile SDKs share one version number.

This SDK was never affected: its token comes from its own platform's push
service and does not change kind.

## 0.7.1

No change to this package beyond the version. Published alongside the Flutter
SDK, which stopped asking apps which push service issued a token and started
reading the token instead — and the three mobile SDKs share one version
number.

This SDK was never affected: it registers the token its own platform's push
service issued, which is unambiguous.

## 0.7.0

No change to this package beyond the version. Published alongside the
Flutter SDK, which needed it: iOS push had never worked through that
package — it registered Firebase's token and Notibase posted it to Apple,
which refused every send — and the three mobile SDKs share one version
number.

This SDK was never affected. It registers the token its own platform's
push service issued, which is what the server now expects every SDK to be
explicit about.
